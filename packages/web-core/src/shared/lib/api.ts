...
import {
  ...
  RuntimeProfile,
  CreateProviderPermissionPolicy,
...
// Provider-independent AI platform API
export const providerPlatformApi = {
  snapshot: async (): Promise<ProviderPlatformSnapshot> => {
    const response = await makeRequest('/api/provider-platform');
    return handleApiResponse<ProviderPlatformSnapshot>(response);
  },

  installPlugin: async (
    data: InstallProviderPluginRequest
  ): Promise<ProviderPlugin> => {
    const response = await makeRequest(
      '/api/provider-platform/plugins/install',
      {
        method: 'POST',
        body: JSON.stringify(data),
      }
    );
    return handleApiResponse<ProviderPlugin>(response);
  },

  createAccount: async (
    data: CreateProviderAccount
  ): Promise<ProviderAccount> => {
    const response = await makeRequest('/api/provider-platform/accounts', {
      method: 'POST',
      body: JSON.stringify(data),
    });
    return handleApiResponse<ProviderAccount>(response);
  },

  createModel: async (
    data: CreateModelDefinition
  ): Promise<ModelDefinition> => {
    const response = await makeRequest('/api/provider-platform/models', {
      method: 'POST',
      body: JSON.stringify(data),
    });
    return handleApiResponse<ModelDefinition>(response);
  },

  createRuntime: async (
    data: CreateRuntimeProfile
  ): Promise<RuntimeProfile> => {
    const response = await makeRequest('/api/provider-platform/runtimes', {
      method: 'POST',
      body: JSON.stringify(data),
    });
    return handleApiResponse<RuntimeProfile>(response);
  },

  createPermissionPolicy: async (
    data: CreateProviderPermissionPolicy
  ): Promise<ProviderPermissionPolicy> => {
    const response = await makeRequest(
      '/api/provider-platform/permission-policies',
      {
        method: 'POST',
        body: JSON.stringify(data),
      }
    );
    return handleApiResponse<ProviderPermissionPolicy>(response);
  },
  ...
  createAgentProfile: async (
    ...
    const response = await makeRequest(
      '/api/provider-platform/agent-profiles',
      {
        method: 'POST',
        body: JSON.stringify(data),
      }
    );
    return handleApiResponse<AgentProfile>(response);
  },
  ...
  validateAssignment: async (
    ...
    const response = await makeRequest(
      '/api/provider-platform/assignments/validate',
      {
        method: 'POST',
        body: JSON.stringify(data),
      }
    );
    return handleApiResponse<AssignmentValidation>(response);
  },

  createAssignment: async (
    data: CreateAssignment
  ): Promise<CreateAssignmentResponse> => {
    const response = await makeRequest('/api/provider-platform/assignments', {
      method: 'POST',
      body: JSON.stringify(data),
    });
    return handleApiResponse<CreateAssignmentResponse>(response);
  },

  startSession: async (
    data: StartProviderSessionRequest
  ): Promise<StartProviderSessionResponse> => {
    const response = await makeRequest(
      '/api/provider-platform/sessions/start',
      {
        method: 'POST',
        body: JSON.stringify(data),
      }
    );
    return handleApiResponse<StartProviderSessionResponse>(response);
  },
};

export const sshHostsApi = {
  list: async (): Promise<SshHostsResponse> => {
    const response = await makeRequest('/api/ssh/hosts');
    return handleApiResponse<SshHostsResponse>(response);
  },

  inspect: async (alias: string): Promise<SshConnectionInfo> => {
    const response = await makeRequest('/api/ssh/inspect', {
      method: 'POST',
      body: JSON.stringify({ alias }),
    });
    return handleApiResponse<SshConnectionInfo>(response);
  },
};

export const aiRoomsApi = {
  list: async (): Promise<AiRoom[]> => {
    const response = await makeRequest('/api/ai-rooms');
    return handleApiResponse<AiRoom[]>(response);
  },

  create: async (data: CreateAiRoom): Promise<AiRoom> => {
    const response = await makeRequest('/api/ai-rooms', {
      method: 'POST',
      body: JSON.stringify(data),
    });
  ...
  snapshot: async (roomId: string): Promise<AiRoomSnapshot> => {
    const response = await makeRequest(`/api/ai-rooms/${roomId}`);
  ...
  prepareRemote: async (roomId: string): Promise<AiRoomSnapshot> => {
    const response = await makeRequest(
      `/api/ai-rooms/${roomId}/prepare-remote`,
      {
        method: 'POST',
      }
    );
  ...
  sync: async (roomId: string): Promise<SyncAiRoomResponse> => {
    const response = await makeRequest(`/api/ai-rooms/${roomId}/sync`, {
      method: 'POST',
    });
  ...
  updateDocument: async (
    ...
    const response = await makeRequest(
      `/api/ai-rooms/${roomId}/documents/${kind}`,
      {
        method: 'PUT',

...
impl PtyService {
    ...
    pub async fn create_session(
        ...
        self.create_command_session(shell, args, working_dir, cols, rows, true)
            .await
    }
    ...
    pub async fn create_process_session(
        ...
        self.create_command_session(program, args, working_dir, cols, rows, false)
            .await
    }
    ...
    async fn create_command_session(
        ...
        cols: u16,
        rows: u16,
        configure_interactive_shell: bool,
    ) -> Result<(Uuid, mpsc::UnboundedReceiver<Vec<u8>>), PtyError> {
        ...
        let result = tokio::task::spawn_blocking(move || {
            ...
            let program_name = program.file_name().and_then(|n| n.to_str()).unwrap_or("");

            if configure_interactive_shell
                && program_name != "powershell.exe"
                && program_name != "pwsh.exe"
            ...
            if configure_interactive_shell && program_name == "zsh" {

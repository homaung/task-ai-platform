...
mod migration_tests {
    ...
    async fn creates_provider_independent_platform_tables() {
        ...
        let table_count: i64 = sqlx::query_scalar(
            "SELECT COUNT(*) FROM sqlite_master WHERE type = 'table' AND name IN ('provider_plugins', 'provider_accounts', 'model_definitions', 'runtime_profiles', 'permission_policies', 'agent_profiles', 'assignments', 'provider_sessions', 'provider_session_events', 'ai_rooms')",
        )
        .fetch_one(&pool)
        .await
        .expect("platform tables should be queryable");
        assert_eq!(table_count, 10);

        let account_columns: Vec<String> =
            sqlx::query_scalar("SELECT name FROM pragma_table_info('provider_accounts')")
                .fetch_all(&pool)
                .await
                .expect("provider account schema should be inspectable");

        assert!(account_columns.contains(&"provider_plugin_id".to_string()));
        assert!(!account_columns.iter().any(|column| {
            ["openai", "anthropic", "google", "codex", "claude"]
                .iter()
